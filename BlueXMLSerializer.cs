...
namespace Lab_9
{
    public class BlueXMLSerializer : BlueSerializer
    {
        ...
        public class ResponseData
        {
            ...
            public ResponseData(Blue_1.Response response)
            {
        ...
        {            
            ...
            var responseDto = new ResponseData(participant);
        ...
        public class WaterJumpDTO
        {
            ...
            {
                Type = jump?.GetType().Name ?? "WaterJump";
                Name = jump?.Name ?? string.Empty;
                Bank = jump?.Bank ?? 0;
                Participants = jump?.Participants != null
                    ? jump.Participants.Select(p => new ParticipantB2DTO(p)).ToArray()
                    : Array.Empty<ParticipantB2DTO>();
            }
        }
        ...
        public override Blue_1.Response DeserializeBlue1Response(string fileName)
        {
            ...
            string path = Path.Combine(FolderPath, fileName);
            var xmlFormatter = new XmlSerializer(typeof(ResponseData));
            using (var stream = new StreamReader(path))
            {
                responseData = (ResponseData)xmlFormatter.Deserialize(stream);
            }
