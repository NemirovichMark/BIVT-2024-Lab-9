...
namespace Lab_9
{
    public class BlueXMLSerializer : BlueSerializer
    {
        ...
        //сериализует объект Response в XML
        public override void SerializeBlue1Response(Blue_1.Response participant, string fileName)
        {
            if (participant == null || string.IsNullOrEmpty(fileName)) return; // проверка
            SelectFile(fileName);

            ResponseSD part = new ResponseSD(participant); // DTO-объект
            XmlSerializer xml = new XmlSerializer(typeof(ResponseSD)); // сериализация в XML с помощью класса XmlSerializer или десереализация
            //typeof получает информацию о классе ResponseSD для обработки данных этого класса
            using (var file = new StreamWriter(fileName)) //StreamWriter класс который используется для записи текстовых данных в файл
            {
                xml.Serialize(file, part);
            }
        }
        
        // сериализует объект WaterJump в XML
        ...
        // сериализует объект Participant в XML
        ...
        // сериализует объект Group в XML
        ...
        public override Blue_1.Response DeserializeBlue1Response(string fileName)
        {
            ...
            ResponseSD part;
            using (var file = new StreamReader(fileName)) // StreamReader для чтения содержимого файла
            {
                part = (ResponseSD)xml.Deserialize(file); // содержимое файла конвертируется в объект типа ResponseSD
            }
        ...
        public override Blue_2.WaterJump DeserializeBlue2WaterJump(string fileName)
        {
            ...
            {
                Blue_2.Participant part2 = new Blue_2.Participant(i.Name, i.Surname); // Создает объект Participant с именем и фамилией длч каждого участника
                foreach (var j in i.Marks)
                {
                    part2.Jump(j); //Восстанавливает оценки (Marks) через метод Jump
                }
        ...
        public override T DeserializeBlue3Participant<T>(string fileName)
        {
            ...
            {
                //Для каждого значения в массиве Penalties (штрафные минуты) вызывает метод PlayMatch, чтобы восстановить историю штрафов участника
                result.PlayMatch(i);
            }
            return (T)result;
        }
        ...
        public override Blue_4.Group DeserializeBlue4Group(string fileName)
        {
            ...
            if (part.ManTeams != null)
            {
                foreach (var part2 in part.ManTeams)
                {
                    ...
                    Blue_4.Team team = new Blue_4.ManTeam(part2.Name);// Создаём объект мужской команды с именем из XML
                    if (part2.Scores != null)
                    {// Если у команды есть результаты матчей, добавляем их
        ...
        public override T DeserializeBlue5Team<T>(string fileName)
        {
            ...
            if (part.Type == "ManTeam") // Если в XML указано, что команда мужская, создаём мужскую команду
            {
                result = new Blue_5.ManTeam(part.Name);
            }

            if (part.Sporsman != null) // Если в XML есть список спортсменов, добавляем каждого в команду
            {
                foreach (var part2 in part.Sporsman)
                {
                    if (part2 == null) continue;

                    var sportsman = new Blue_5.Sportsman(part2.Name, part2.Surname); // Создаём объект спортсмена с именем и фамилией
                    sportsman.SetPlace(part2.Place);// Устанавливаем место спортсмена
