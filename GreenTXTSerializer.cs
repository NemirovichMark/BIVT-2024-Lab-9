...
namespace Lab_9
{
    public class GreenTXTSerializer : GreenSerializer
    {
        ...
        {
            string[] lines = File.ReadAllLines(Path.Combine(FolderPath, $"{fileName}.{Extension}"));
        ...
        {
            string[] lines = File.ReadAllLines(Path.Combine(FolderPath, $"{fileName}.{Extension}"));
            Green_4.Discipline discipline = lines[1].EndsWith("LongJump")
                ? new LongJump()
                : new HighJump();
            ...
            for (int i = 3; i < 3 + count; i++)
            {
                ...
                Participant participant = new Participant(parts[0], parts[1]);

                foreach (double jump in parts[2].Split(',').Select(double.Parse))
                {
        ...
        public override T DeserializeGreen5Group<T>(string fileName)
        {
            ...
            {
                string[] parts = lines[i].Split('|');
                Green_5.Student student = new Green_5.Student(parts[0], parts[1]);
