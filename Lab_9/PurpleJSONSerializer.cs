...
namespace Lab_9
{
    public class PurpleJSONSerializer : PurpleSerializer
    {
        ...
        public override T DeserializePurple1<T>(string fileName)
        {
            ...
            {
                return DeserializePurple1Competition(data) as T;
            }
            return null;
        }
        ...
        private Purple_1.Judge DeserializePurple1Judge(JToken judge)
        {
            string name = judge["Name"].ToObject<string>();
            int[] marks = judge["Marks"].ToObject<int[]>();

            var newJudge = new Purple_1.Judge(name, marks);
            return newJudge;
        }
        private Purple_1.Competition DeserializePurple1Competition(JToken competition)
        {
            var participants = competition["Participants"]["$values"];
            var judges = competition["Judges"]["$values"].ToObject<Judge[]>();
        ...
        private Purple_5.Research DeserializePurple5Reserch(JToken research)
        {
            ...
            foreach(JObject response in responses)
            {
                ...
                for (int i = 0; i < responseValues.Length; i++)
                    if (string.IsNullOrEmpty(responseValues[i])) responseValues[i] = null;
