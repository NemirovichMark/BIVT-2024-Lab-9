...
using static Lab_9.PurpleJSONSerializer;
...
namespace Lab_9
{
    public class PurpleJSONSerializer : PurpleSerializer
    {
        ...
        private void SaveJson<T>(T data)
        {
        ...
        public override T DeserializePurple1<T>(string fileName)
        {
            ...
            {
                var result = JsonConvert.DeserializeObject<Purple_1_ParticipantDto>(json);
                return DeserializeParticipantPurple1(result) as T;
            }
            else if (typeof(T) == typeof(Purple_1.Judge))
            {
                var result = JsonConvert.DeserializeObject<Purple_1_JudgeDto>(json);
                return DeserializeJudgePurple1(result) as T;
            }
            else if (typeof(T) == typeof(Purple_1.Competition))
            {
                var result = JsonConvert.DeserializeObject<Purple_1_CompetitionDto>(json);
        ...
        public override T DeserializePurple2SkiJumping<T>(string fileName)
        {
            ...
            var json = JsonConvert.DeserializeObject<Purple_2_SkiJumpingDto>(s);
            Purple_2.SkiJumping ski;
            if (json.Standart == 100)
            {
                ski = new Purple_2.JuniorSkiJumping();
            }
            else if (json.Standart == 150)
            {
            ...
            foreach (var p in participants)
            {
                ski.Add(DeserializeParticipantPurple2(p, json.Standart));
            }
            return ski as T;
        }
        ...
        public override T DeserializePurple3Skating<T>(string fileName)
        {
            SelectFile(fileName);
            string s = File.ReadAllText(FilePath);
            var json = JsonConvert.DeserializeObject<Purple_3_SkatingDto>(s);
            Purple_3.Skating skate;
            if (json.Type == nameof(Purple_3.IceSkating))
            {
                skate = new Purple_3.IceSkating(json.Moods, false);
            }
            else if (json.Type == nameof(Purple_3.FigureSkating))
            {
                skate = new Purple_3.FigureSkating(json.Moods, false);
            }
            else
            {
                return default;
        ...
        public override Purple_4.Group DeserializePurple4Group(string fileName)
        {
            SelectFile(fileName);
            string s = File.ReadAllText(FilePath);
            var json = JsonConvert.DeserializeObject<Purple_4_Group>(s);
            Purple_4.Group group = new Purple_4.Group(json.Name);
            var sportsmen = json.Sportsmen;
        ...
        public override Purple_5.Report DeserializePurple5Report(string fileName)
        {
            SelectFile(fileName);
            string json = File.ReadAllText(FilePath);
            Purple_5_ReportDto r = JsonConvert.DeserializeObject<Purple_5_ReportDto>(json);
